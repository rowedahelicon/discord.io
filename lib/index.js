 ...
 * @arg {String} options.intents - The intents of the account you wish to log in with.
...
DCP.uploadFile = function(input, callback) {
	...
	//if( input.components ) console.log( input.components );
 ...
 * @arg {Object} [input.components] - An embed object to include
 * @arg {Object} [input.embeds] - An embed object to include
...
DCP.sendMessage = function(input, callback) {
	var message = generateMessage(input.message || '', input.embed, input.components, input.embeds);
	message.tts = (input.tts === true);
	message.nonce = input.nonce || message.nonce;
	message.message_reference = input.message_reference || null;

	//if (input.message_reference) message.message_reference = input.message_reference;
...
DCP.createThread = function(input, callback) {

	var message = [];
	message.name = input.title;
	message.auto_archive_duration = 10080;
	message.type = ( input.private ? 12 : 11 ),
	message.invitable = input.invitable || true;
	message.rate_limit_per_user = input.rate_limit_per_user || 0;

	createThread(this, input.to, message, callback);
}

DCP.createThreadFromMessage = function(input, callback) {

	var message = generateThread(input);

	createThreadFromMessage(this, input.to, input.id, message, callback);
}
 ...
 * @arg {Object} [input.components] - The new Discord Embed object
 * @arg {Object} [input.embeds] - The new Discord Embed object
 */
DCP.editMessage = function(input, callback) {
	this._req('patch', Endpoints.MESSAGES(input.channelID, input.messageID), generateMessage(input.message || '', input.embed, input.components, input.embeds), function(err, res) {
 ...
 * @arg {Snowflake} input.messageID
 */
DCP.returnAudit = function(input, callback) {
	this._req('get', Endpoints.AUDIT_LOG(input.serverID), function(err, res) {
		handleResCB("Unable to return audit log", err, res, callback);
	});
};
...
DCP.scheduleEvent = function(input, callback) {
	this._req('post', Endpoints.EVENTS(input.serverID), { channel_id: input.channel_id, entity_metadata: input.entity_metadata, name: input.name, privacy_level: input.privacy_level, scheduled_start_time: input.scheduled_start_time, scheduled_end_time: input.scheduled_end_time, description: input.description, entity_type: input.entity_type, image: input.image }, function(err, res) {
		handleResCB("Could not schedule event", err, res, callback);
	});
};

DCP.scheduledEvents = function(input, callback) {
	this._req('get', Endpoints.EVENTS(input.serverID), {}, function(err, res) {
		handleResCB("Could not retrieve events", err, res, callback);
	});
};
...
function generateMessage(message, embed, components, embeds) {
	return {
		type: 0,
...
function generateThread(thread) 
{
	return {
		name: thread.title,
		auto_archive_duration: thread.auto_archive_duration || 10080,
		type: ( thread.private ? 11 : 12 ),
		invitable: thread.invitable || true,
...
/* - Functions - Threads */
function createThread(client, to, message, callback) {
	console.log( message );
	resolveID(client, to, function(channelID) {
		client._req('post', Endpoints.THREADS(channelID), message, function(err, res) {
			handleResCB("Unable to create thread", err, res, callback);
		});
	});
}

function createThreadFromMessage(client, to, id, message, callback) {
	console.log( message );
	resolveID(client, to, function() {
		client._req('post', Endpoints.THREADS(to, id), message, function(err, res) {
			handleResCB("Unable to create thread", err, res, callback);
		});
	});
}
	...
	switch (message.t) {
		case "READY":
			...
			if (client.bot) client.getOauthInfo(function(err, res) {
				...
				client.inviteURL = "https://discord.com/oauth2/authorize?client_id=" + res.id + "&scope=bot&permissions=98303";
			});
		...
		case "GUILD_MEMBER_UPDATE":
			if (!_data.guild_id) break;
			if (typeof(client.servers[_data.guild_id].members) == 'undefined') { break; } //TODO: Temp-Fix? Bot got a guild member update before finished logging in
			//if (!_data.guild_id) break; //TODO: Temp-Fix? Bot got a guild member update before finished logging in
...
function Member(client, server, data) {
	...
	['username', 'discriminator', 'bot', 'avatar', 'game'].forEach(function(k) {
		...
		if (typeof(this) == "undefined") return; //I added thjs, no idea what broke our member list?
...
(function () {
	...
	Endpoints = Discord.Endpoints = {
		...
		THREADS: function(channelID, messageID) {
			return  this.MESSAGES(channelID, messageID) + "/threads";
		},
		AUDIT_LOG: function(serverID) {
			return  this.SERVERS(serverID) + "/audit-logs";
		},
		...
		//UNsure
		EVENTS: function(serverID) {
			return  this.SERVERS(serverID) + "/scheduled-events";
		}
